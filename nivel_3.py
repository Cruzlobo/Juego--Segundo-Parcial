import pygame, sys
import csv
from Class_disparo_boss import *
...
#Fondo
fondo = pygame.image.load(r"Sprites\mapa boss\maPA.png").convert()
...
personaje = Personaje(diccionario, (130,100), 50,970,10, 7)

monedas = pygame.sprite.Group()  
moneda = Moneda(700, 570)
moneda2 = Moneda(270, 240) 
...
botella3 = Item(875, 365)
items.add(botella,botella2,botella3)
...
imagen_plataforma2 = pygame.image.load(r"Sprites\OBJECTOS\PLATAFORMA\0.png")
imagen_plataforma= pygame.transform.scale(imagen_plataforma,(W,9))
imagen_plataforma2= pygame.transform.scale(imagen_plataforma2,(100,10))
...
piso2 = crear_plataforma("",W,100,0,1100,False)
piso["rectangulo"].top = personaje.rectangulo_principal.bottom
plataforma_invisible = crear_plataforma("", 200, 30, 208 , 835, False)      
plataforma_invisible1 = crear_plataforma("", 100, 120, 508 , 960, False)      
plataforma_invisible2 = crear_plataforma("", 100, 30, 0 , 700, False)
plataforma_invisible3 = crear_plataforma("", 450, 10, 1130 , 550, False)
plataforma_invisible4 = crear_plataforma("", 600, 10, 180, 630, False)
plataforma_invisible5 = crear_plataforma("", 220, 10, 453, 460, False)
plataforma_invisible6 = crear_plataforma("", 220, 10, 223, 295, False)
plataforma_invisible7 = crear_plataforma("", 100, 10, 845, 380, False)
plataforma_invisible8 = crear_plataforma("", 150, 150, 1200, 910, False)
plataforma_invisible9 = crear_plataforma("", 250, 310, 1400, 750, False)

plataformas = [piso, plataforma_invisible,
            plataforma_invisible1,plataforma_invisible2,
            plataforma_invisible3,plataforma_invisible6,
            plataforma_invisible4,plataforma_invisible5,
            plataforma_invisible7,plataforma_invisible8,
            plataforma_invisible9,piso2]


colisiones = Colisiones()
duracion_temporizador = 180 * 1000  # 90 segundos
...
mira_Derecha = None
max_enemigos = 2
pygame.mixer.music.load(r'Sonidos\Mapa\Boss.wav')
...
csv_file_path = "puntajes3.csv"  
csv_header = ["Jugador", "Puntuacion"] 
boss_group = pygame.sprite.Group()
boss = Boss(personaje,boss_group)
boss_group.add(boss)
disparos_boss = [] 
max_enemigos = 10
...
while True:
    ...
    if tiempo_transcurrido >= duracion_temporizador:
        ...
        personaje = Personaje(diccionario, (150,120), 50,970,10, 10)
        lista_enemigos = Enemigo.crear_lista(plataformas)
        tiempo_inicio = pygame.time.get_ticks()
...
################################################################### 
    ...
    PANTALLA.blit(imagen_plataforma,(0,1070))
    PANTALLA.blit(imagen_plataforma2,(0,700))
    PANTALLA.blit(imagen_plataforma2,(848,380))
    ...
    boss_group.update()
    boss_group.draw(PANTALLA)
    for moneda in monedas.sprites():
        PANTALLA.blit(moneda.image, moneda.rect)
    for disparo in boss.lista_disparos:
        PANTALLA.blit(disparo.image, disparo.rect)
    personaje.actualizar(PANTALLA, plataformas)
    Colisiones.verificar_colision_disparo_boss(personaje,boss,PANTALLA)
    Colisiones.verificar_colision_disparo_personaje_boss(personaje,boss)
    colisiones.verificar_colision_personaje_enemigo(personaje, lista_enemigos,PANTALLA)
    Colisiones.verificar_colision_disparo_personaje_nivel3(personaje.lista_proyectiles, lista_enemigos)
    ...
    Colisiones.dibujar_barra_salud_boss(None, PANTALLA,boss)
    ...
    if personaje.vida <= 0:
        ...
        personaje = Personaje(diccionario, (150,120), 50,850,10, 7)
        lista_enemigos = Enemigo.crear_lista(plataformas)
            ...
            if enemigo.esta_muerto:
                ...
                if tiempo_transcurrido > enemigo.TIEMPO_ANIMACION_MUERTE:
                    ...
                    if len(lista_enemigos) < max_enemigos:
                        nuevo_enemigo = Enemigo.generar_enemigo_aleatorio(10)
    ...
    if boss.esta_muerto:
        ...
        dibujar_texto("Has salvado a la tierra!!",fuente,(255,255,255),450,700,PANTALLA)
    ...
    if get_mode() == True:
        ...
        rectangulo_boss = obtener_rectangulos(
                boss.rect.x,boss.rect.y,
                boss.rect.width,
                boss.rect.height)
        pygame.draw.rect(PANTALLA, "green", rectangulo_boss["main"], 2)
        pygame.draw.rect(PANTALLA, "blue", rectangulo_boss["bottom"],2)
        pygame.draw.rect(PANTALLA, "red", rectangulo_boss["right"],2)
        pygame.draw.rect(PANTALLA, "red", rectangulo_boss["left"],2)
        pygame.draw.rect(PANTALLA, "red", rectangulo_boss["top"],2)
        for disparo in boss.lista_disparos:
            rectangulo_ajustado_boss = disparo.rect.move(-disparo.rect.width // 8, -disparo.rect.height // 16)
            pygame.draw.rect(PANTALLA, "yellow", rectangulo_ajustado_boss, 1)
